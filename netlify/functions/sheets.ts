// Netlify serverless function — netlify/functions/sheets.ts
// Identical logic to api/sheets.ts, adapted for Netlify handler format
...
export const handler = async (event: any) => {
  ...
  try {
    ...
    if (event.httpMethod === 'POST') {
      const { name, score, date, roundDetail } = JSON.parse(event.body || '{}');
